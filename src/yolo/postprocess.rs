//! Postprocessing for YOLOv8 pose predictions with Burn
//!
...
pub fn postprocess<B: Backend>(
    ...
    let predictions = predictions.to_device(&device);

    // Get tensor data
    let data = predictions.to_data();
    // data.shape is Vec<usize>, directly use it
    ...
    // Convert tensor data to vec
    let values: Vec<f32> = data.to_vec().expect("Failed to convert tensor to vec");
    ...
    for idx in 0..num_preds {
        // Extract prediction for this detection
        // Tensor is stored in row-major order: predictions[i][j] = values[i * num_preds + j]
        let mut pred_vec = Vec::with_capacity(pred_size);
        for i in 0..pred_size {
            pred_vec.push(values[i * num_preds + idx]);
        }

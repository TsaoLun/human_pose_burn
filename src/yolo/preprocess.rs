//! Image preprocessing for YOLOv8 inference with Burn
//!
...
pub fn preprocess<B: Backend>(
    ...
    // Convert to CHW format directly to avoid permutation
    // Organize data as [C, H, W] where C channels are interleaved
    let mut chw_data = Vec::with_capacity(3 * target_h * target_w);
    ...
    for c in 0..3 {
        for y in 0..target_h {
            for x in 0..target_w {
                ...
                // Normalize to [0, 1] during conversion
                chw_data.push(resized[idx] as f32 / 255.0);
            }
        }
    }

    // Create tensor with shape (1, 3, H, W)
    let data = TensorData::new(chw_data, Shape::from([1, 3, target_h, target_w]));

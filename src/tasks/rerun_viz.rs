...
impl RerunPoseViz {
    ...
    fn log_poses(&self, poses: &CuPoses) -> CuResult<()> {
        info!("RerunPoseViz: Received {} poses", poses.len());
        ...
        if !all_keypoints.is_empty() {
            info!("RerunPoseViz: Logging {} keypoints, {} skeleton lines, {} boxes",
                all_keypoints.len(), skeleton_strips.len(), box_centers.len());
            ...
            info!("RerunPoseViz: No keypoints to log");
        }
